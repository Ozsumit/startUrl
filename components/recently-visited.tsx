...
const RecentlyVisited = () => {
  const [recentSites, setRecentSites] = useState<Website[]>([])
  ...
  // This function gets recently visited sites
  useEffect(() => {
    const loadSites = () => {
      ...
      const sortedSites = [...allSites]
        .filter((site) => site.lastVisited) // Only include sites that have been visited
    ...
    return (
      <div className="grid place-items-center p-8 bg-background/40 backdrop-blur-md rounded-lg">
        <p className="text-muted-foreground text-center">
          No recently visited sites yet. Start browsing to see them here!
        </p>
      </div>
    )
  }
    ...
    >
      ...
      {site.lastVisited && (
        <span className="text-xs text-muted-foreground">
          {formatDistanceToNow(new Date(site.lastVisited), { addSuffix: true })}
        </span>
      )}
    </div>
  )
...
export default memo(RecentlyVisited)
